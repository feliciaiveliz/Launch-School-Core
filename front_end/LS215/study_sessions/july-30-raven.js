...
Mental Model:
  - return the sum of all elements that are common to all three arrays. duplicate elements must have the same number of occurences across all three arrays to be added to the result.
  
Data structure:
  - use an array to store common elements
  - return a number that is the sum of those elements
  - use object(s) to store the number of occurences of each element in each array, then add     element * lowest number of occurences for each common element to result array
  
Algorithm:
  - create a nested array from the arrays 
  - declare a variable to keep track of the sum, starting at 0
  - iterate over the nested array, transforming each array to an object => array of objects
  ...
  - check that comparison between the three values at that key for equality
    - if the object[key] is truthy in all objects, then key (as a number) * min value is added to the sum 
