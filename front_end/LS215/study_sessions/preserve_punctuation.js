    ...
    if (string.match(/[^a-z0-9]/i)) {
      ...
      string.split('').forEach((character, index) => {
        if (character.match(/[^a-z0-9]/gi)) {
          puncObj[index] = character;
        }
        
        string = string.replace(/[^a-z0-9]/gi, '').split('').reverse().join('');
        
        Object.keys(puncObj).forEach(indexKey => {
          string = string.split('');
          string.splice(indexKey, 0, puncObj[indexKey]);
        });
