...
let todoList = (function() {
  let list = [];
  const TEMPLATE = ['id', 'title', 'completed', 'month', 'year', 'description'];
  ...
  return { 
    ...
    updateTodo(id, object) {
      ...
      properties.forEach(property => {
        if (!TEMPLATE.includes(property) || (property === 'id' && 
        !existingIds.includes('id'))) {
          return "Cannot update todo.";
        } else {
          todo[property] = object[property];
        }
      });
    },
...
let todoManager = (function() {
  function copyList(todoList) {
    todoList = JSON.stringify(todoList);
    let copiedTodoList = JSON.parse(todoList);
    return copiedTodoList;
  }

  return {
    init(todoListObject) {
      this.todos = todoListObject.getList();
      return this;
    },

    allTodos() {
      return copyList(this.todos);
    },

    completedTodos() {
      let todosCompleted = this.todos.filter(todo => todo.completed === true);
      return copyList(todosCompleted);
    },
...
// NEW TESTS ADDED AND CHANGES MADE/VERIFIED
todoList.updateTodo(1, { id: 0, newProperty: `I'm a new property!` }); // cannot update todo
todoList.updateTodo(0, { completed: true });                           // cannot update todo
todoList.updateTodo(2, { completed: true });                           // updates todo
todoList.updateTodo(3, { completed: true });                           // updates todo
todoList.updateTodo(4, { completed: true });                           // updates todo
todoManager.allTodos(); // returns newly updated todos and 'cannot update todos' for invalid properties

todoManager.completedTodos()[0].id = 1;
todoManager.allTodos();  // shows newly updated todos from lines '401-402' and also does not allow for changes to the todo object
// [
//   {
//     id: 1,
//     title: 'Mow lawn',
